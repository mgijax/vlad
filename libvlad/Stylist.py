...
class Stylist(object):
	...
	if dolabel and rslts and not n.is_nsroot:
		...
		for r in rslts:
		    ...
		    qsc = ' '.join(map(lambda x:'%s'%x, self.qsid2color[r.qsid]))
		    if self.doPercents:
	...
	for (sgn, attrs) in sgattrs.items():
	    self.dotdag.setSubgraph(sgn) # creates the sg path
	    for which, wattrs in attrs.items():	# now set sg attributes
