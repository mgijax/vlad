...
class Vlad(object):
	...
	for sn in config.sections():
		...
		o.order = config.get(sn, 'order', 100)
	...
	return filter(None,re.split(r'[^a-zA-Z0-9_:.-]+', idString))

    def parseExcludeList(self, value):
        return filter(None, re.split( r'\W+', value.upper() ))
	...
	for ns,rslts in self.results.items():
	  if len(rslts) == 0:
	    del self.results[ns]
	...
	rv = "".join(
	  map( lambda m : '<div class="message %s-message">%s</div>' % (m[0],m[1]), self.messages))
	if clear:
	    self.messages = []
	return rv
...
class VladCGI(Vlad):
	...
	if value is None:
	    return None
        vals = map(float,value.replace(","," ").replace("x"," ").split())
	if len(vals) == 0:
	    return None
	if len(vals) == 1:
	    vals.append(vals[0])
