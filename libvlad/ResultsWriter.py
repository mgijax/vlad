...
class ResultsWriter(object):
	...
	self.filesWritten.append(fname)
        if self.fname == "-":
	    ofd = sys.stdout
        elif type(fname) is types.StringType:
            ofd = open(fname, 'w')
	elif hasattr(fname, "write") and callable(getattr(fname, "write")):
	    ofd = fname
	else:
	    raise RuntimeException("Could not open file:" + str(fname))
	return ofd


    def getConfig(self, cfgParser):
	self.columnLegend = map(lambda x:x.split(":"), 
		 COLUMNLEGEND.split("|"))
	self.colName2Legend = {}
	...
	if len(dbobjs) == 0:
	    return None
        if self.batchUrl:
	    idlist = ' '.join(map(lambda x: x.getId(), dbobjs))
	    label = ", ".join( map(lambda x: x.getSymbol(), dbobjs))
	    return HTML_BATCH_LINK_TMPLT % (idlist, label)
	else:
	    return None

    def getHeaderLabels(self):
	return self.checkRow(map(lambda x:x[0], self.columnLegend))
	...
	if format == 'html':
	    ...
	    id = result.term.id
	    symbols = COMMA.join(map(lambda x:x.getSymbol(), dbobjs))
	qid = result.qsid
...
class HTMLWriter(ResultsWriter):
	...
	if type(tdTags) is types.StringType:
	    tdTags = [ tdTags ] * len(lst)
	tdEndTags = map(lambda x:x.split()[0], tdTags)
	...
	if self.columnLegend is None:
	    ...
	    content = "".join(
	        map(lambda x:"<dt>%s</dt><dd>%s</dd>"%(x[0],x[1]), self.columnLegend))
	...
	links = map(self.makeLink, nss, map(lambda x:'#'+x, nss))
...
class DOTWriter(ResultsWriter):
	...
	terms = set()
	if type(self.cutoff) is types.IntType:
	    if self.cutoff > 0:
		# keep first N terms
		for r in nsresults:
		...
		for r in nsresults:
			...
			if len(terms) >= nterms :
			    break
	elif type(self.cutoff) is types.FloatType:
	    for r in nsresults:
		if self.vlad.options.analysis == "percentage":
